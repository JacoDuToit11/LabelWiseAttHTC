...
cnn_model_types = ['JointLaatModel']
...
# use_previous_level_output_scores = True
use_previous_level_output_scores = False
...
def main():
    ...
    global concat_all_previous_levels
    for temp_cnn_model_type in cnn_model_types:
        ...
        for dataset_name in dataset_names:
            ...
            for temp_concat_all_previous_levels in concat_all_previous_levels_list:
                concat_all_previous_levels = temp_concat_all_previous_levels
                                ...
                                if use_different_lrs:
                                    for bert_lr in bert_lrs:
                                        for attention_lr in attention_lrs:
                                            train(dataset_name, level_num_labels, depth2label, train_dataset, val_dataset, test_dataset, num_labels, random_seed, batch_size, 0, dropout_prob, bert_lr, attention_lr)
                                else:
                                    for learning_rate in learning_rates:
                                        train(dataset_name, level_num_labels, depth2label, train_dataset, val_dataset, test_dataset, num_labels, random_seed, batch_size, learning_rate, dropout_prob)
                    else:
                        if low_resource:
                            if dataset_name == 'WOS':
...
def train(dataset_name, level_num_labels, depth2label, train_dataset, val_dataset, test_dataset, num_labels, random_seed, train_batch_size, learning_rate, dropout_prob, bert_lr = 0, attention_lr = 0):
    ...
    if cnn_model_type == 'JointLaatModel':
    ...
    if use_wandb:
        ...
        wandb_logger.experiment.config.update({'dataset': dataset_name, 'classifier': 'hier_local', 'bert_model': model_name, 'batch_size': train_batch_size, 'learning_rate': learning_rate, 'dropout_prob': dropout_prob,
                                                ...
                                                'bert_lr': bert_lr, 'attention_lr': attention_lr, 'linear_warmup_ratio': linear_warmup_ratio,
    ...
    def configure_optimizers(self):
        if layer_wise_lr_decay:
            ...
            for idx, name in enumerate(layer_names):
                if cnn_model_type == 'JointLaatModel':
                    if name.split('.')[0] in 'attention_model' or name.split('.')[0] in 'projection_linears':
