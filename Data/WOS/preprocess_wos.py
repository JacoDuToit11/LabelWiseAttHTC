...
# NOTE these are the overlapping codes.
...
def get_data_from_meta():
    # TODO look into not cleaning the data
    clean_input = True
    ...
    for line in origin_txt[1:]:
        ...
        # NOTE This is done for the case where there are more than 2 labels for a single code, so they set the code with the most samples as the main label.
        # TODO can maybe be done in a better way?
        for i in label_check[code]:
            if stats[sample_label[0]][i] > stats[sample_label[0]][sample_label[1]]:
                sample_label[1] = i
                break
        
        # TODO change acronum to full names such as CS to Computer Science, MAE to Mechanical and Aerospace Engineering, ECE to Electrical and Computer Engineering, etc.
